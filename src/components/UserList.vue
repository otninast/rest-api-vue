<template>
  <v-container>
    <v-layout  row wrap justify-start align-center>
      <v-flex xs12 md6 lg4 pa-2 v-for="(user, userlabel) in userlist" :key="userlabel">
        <v-card class="my-4">
          <v-card-title primary-title class="indigo white--text">
            <h1>{{user.username}}</h1>
          </v-card-title>
          <v-card-text>
            <v-layout column wrap align-center justify-center mb-2>

              <v-flex xs4 md4 lg4 pa-3 class="">
                <v-layout column justify-center align-center>
                  ...
                  <v-flex xs12 md12 lg12>
                    <v-avatar size="120" color="grey">
                      <img :src="user.profile.profile_image" class="elevation-4">
                    </v-avatar>
                  </v-flex>
                </v-layout>
              </v-flex>
